            ...
            <div class="bg-surface-white border border-outline-variant/40 rounded-[20px] shadow-[0px_1px_4px_rgba(30,42,74,0.06)] p-6 mt-4 overflow-hidden">
                ...
                <div x-show="!editing"
                    ...
                    @if($course->description)
                        <div class="prose prose-sm max-w-none text-on-surface overflow-x-hidden break-words">
                            {!! $course->description !!}
                        </div>
                    @else
                ...
                @else
                    <ul class="divide-y divide-outline-variant/20">
                        @foreach($course->units as $unit)
                            <li x-data="{ confirmingDelete: false }"
                                class="flex items-center gap-4 px-6 py-3.5 hover:bg-surface-container-low/40 transition-colors duration-200">
                                ...
                                {{-- Title --}}
                                <span class="flex-1 text-sm font-medium text-on-surface truncate">
                                    {{ $unit->title }}
                                </span>
                                ...
                                <div class="flex items-center gap-1.5 shrink-0">
                                    ...
                                    <div x-show="confirmingDelete" x-cloak
                                        ...
                                        <span class="text-xs text-on-surface-variant whitespace-nowrap">Delete?</span>
                                        <form method="POST"
                                              action="{{ route('admin.units.destroy', $unit->id) }}">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                                                    aria-label="Confirm delete {{ $unit->title }}"
                                                    class="inline-flex items-center justify-center w-8 h-8
                                                           border border-error/40 rounded-[12px] text-error bg-error/5
                                        ...
                                        <button type="button"
                                                ...
                                                class="inline-flex items-center justify-center w-8 h-8
                                                       border border-outline-variant/60 rounded-[12px] text-on-surface-variant
