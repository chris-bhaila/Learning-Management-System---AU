...
<div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-8">
    <div>
        ...
        <p class="mt-1 text-sm text-on-surface-variant">
            All class enrollment tokens{{ $teacher ? ' for ' . $teacher->name : '' }}.
        </p>
    </div>
</div>
...
<x-card class="mb-6 animate-fade-up">
    <div class="px-6 py-4 border-b border-outline-variant/20">
        ...
        <p class="text-xs text-on-surface-variant mt-0.5">
            Filter class tokens by teacher.
        </p>
    </div>
    <div class="p-5">
        @if($teachers->isEmpty())
        ...
        @else
            <div class="max-w-sm">
                ...
                <select
                    id="teacher-select"
                    onchange="if(this.value) window.location='{{ route('admin.tokens.class') }}?teacher_id=' + this.value"
...
@if(!$teacher)
    ...
    <div class="flex flex-col items-center gap-3 py-20 text-center animate-fade-up">
        <div class="w-14 h-14 rounded-2xl bg-surface-container flex items-center justify-center">
            <span class="material-symbols-outlined text-[28px] text-outline animate-float">group</span>
        </div>
        <p class="text-base font-semibold text-on-surface">Select a teacher to view their class tokens</p>
        <p class="text-sm text-on-surface-variant max-w-xs">
            Choose a teacher from the dropdown above to browse all of their class enrollment tokens.
        </p>
    </div>
    ...
    <x-card class="overflow-hidden animate-fade-up">
        @if($tokens->isEmpty())
            <div class="py-16 flex flex-col items-center gap-2 text-center px-4">
                ...
                <p class="text-sm font-semibold text-on-surface">No class tokens yet</p>
                <p class="text-xs text-on-surface-variant max-w-xs">
                    Generate class tokens from the
