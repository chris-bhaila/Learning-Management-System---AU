...
<form
    ...
    x-data="{
        ...
        onSubmit(event) {
            ...
            history.replaceState(null, '', '{{ route('teacher.courses.index') }}');
        },
    }"
>
    @csrf
    ...
    <div class="bg-surface-white border border-outline-variant/40 rounded-[20px] shadow-[0px_1px_4px_rgba(30,42,74,0.06)] p-6 animate-fade-up mb-4">
        ...
        <div>
            ...
            <div class="flex items-center gap-0.5 px-3 py-2 bg-surface-container-low
                ...
                <button type="button" onclick="tiptap.chain().focus().setHorizontalRule().run()" title="Divider"
                        class="w-8 h-8 flex items-center justify-center rounded-lg text-on-surface-variant
                               hover:bg-surface-container hover:text-primary transition-colors cursor-pointer">
    ...
    <div class="bg-surface-white border border-outline-variant/40 rounded-[20px] shadow-[0px_1px_4px_rgba(30,42,74,0.06)] p-6 animate-fade-up mb-4">
        ...
        @if($groups->isEmpty())
            <p class="mt-2 text-xs text-outline">
                ...
                You can organise courses into groups from the
                <a href="{{ route('teacher.courses.index') }}"
                   class="underline hover:text-primary transition-colors cursor-pointer">My Courses</a>
                page once groups are set up.
            </p>
        @endif
    </div>
