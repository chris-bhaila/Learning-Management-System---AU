...
<div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-5 animate-fade-up">
    ...
    <div class="bg-surface-white border border-outline-variant/40 rounded-[20px] p-5 flex items-start gap-4
                shadow-[0px_2px_8px_rgba(30,42,74,0.06)] hover:shadow-[0px_8px_24px_rgba(30,42,74,0.10)] transition-shadow duration-200">
        ...
        <div class="min-w-0">
            ...
            <p class="text-xs text-on-surface-variant mt-0.5">In your class</p>
        </div>
    </div>
    ...
    <div class="bg-surface-white border border-outline-variant/40 rounded-[20px] p-5 flex items-start gap-4
        ...
        <div class="min-w-0">
            <p class="text-[11px] font-semibold tracking-widest text-outline uppercase">Total Courses</p>
            <p class="text-2xl font-bold text-primary mt-0.5" style="font-family: var(--font-display);">
                {{ number_format($stats['total_courses']) }}
            </p>
            <p class="text-xs text-on-surface-variant mt-0.5">Created by you</p>
        </div>
    </div>
    ...
    <div class="bg-surface-white border border-outline-variant/40 rounded-[20px] p-5 flex items-start gap-4
        ...
        <div class="w-11 h-11 rounded-xl bg-gold/20 flex items-center justify-center shrink-0">
            <span class="material-symbols-outlined text-[22px]" style="color: var(--color-on-gold);">key</span>
        </div>
        <div class="min-w-0">
            <p class="text-[11px] font-semibold tracking-widest text-outline uppercase">Active Tokens</p>
            <p class="text-2xl font-bold text-primary mt-0.5" style="font-family: var(--font-display);">
                {{ number_format($stats['active_tokens']) }}
            </p>
            <p class="text-xs text-on-surface-variant mt-0.5">Accepting enrollments</p>
        </div>
    </div>
...
<div class="grid grid-cols-1 xl:grid-cols-3 gap-6 animate-fade-up">
    ...
    {{-- Course Management Table (2/3 width) --}}
    <div class="xl:col-span-2 bg-surface-white border border-outline-variant/40 rounded-[20px] shadow-[0px_1px_4px_rgba(30,42,74,0.06)] overflow-hidden">
        ...
        <div class="flex items-center justify-between px-6 py-4 border-b border-outline-variant/30">
            ...
            <a href="{{ Route::has('teacher.courses.index') ? route('teacher.courses.index') : '#' }}"
               class="text-xs font-medium text-on-surface-variant hover:text-primary transition-colors duration-150 flex items-center gap-1 cursor-pointer">
                View all
                                ...
                                <div class="flex flex-col items-center py-12 gap-3">
                                    ...
                                    <a href="{{ Route::has('teacher.courses.create') ? route('teacher.courses.create') : '#' }}"
                                       class="mt-1 inline-flex items-center gap-1.5 px-4 py-2 bg-gold text-primary
                                              text-sm font-semibold rounded-[24px] hover:bg-gold/90 transition-colors duration-150 cursor-pointer">
