            ...
            <div
                x-data="{
                    ...
                    onUploadSubmit() {
                        ...
                        this.uploading = true;
                        document.getElementById('upload-unit-file-form').requestSubmit();
                    },
                ...
                {{-- Upload section (teacher/admin only — students use a separate view) --}}
                <div class="px-6 py-4 border-b border-outline-variant/20 bg-surface-container-low/40">
                    ...
                    <div class="flex flex-col sm:flex-row items-start sm:items-center gap-2">
                        ...
                        <input id="unit-file-input" type="file" name="file"
                               form="upload-unit-file-form"
                               @change="onFileChange"
                               class="sr-only"
                ...
                @if($unit->files->isEmpty())
                    <div class="py-10 flex flex-col items-center gap-2 text-center px-4">
                        ...
                        <p class="text-xs text-on-surface-variant">Upload a file above to attach it to this unit.</p>
                    </div>
                @else
                    <ul class="divide-y divide-outline-variant/20">
                        @foreach($unit->files as $file)
                            ...
                            <li class="flex items-center gap-3 px-6 py-3.5 min-w-0
                                ...
                                <button type="button"
                                        onclick="confirmDelete({{ Js::from($file->original_name ?? $file->filename) }}, document.getElementById('delete-file-form-{{ $file->id }}'))"
                                        class="shrink-0 inline-flex items-center gap-1 text-xs font-medium
...
{{-- Standalone file upload form — outside #edit-unit-form; file input and submit button
     reference this form via the HTML5 form= attribute so it submits here, not to the edit form. --}}
<form id="upload-unit-file-form" method="POST" action="{{ route('files.store') }}"
      enctype="multipart/form-data" class="hidden">
    @csrf
    <input type="hidden" name="fileable_type" value="App\Models\Unit">
    <input type="hidden" name="fileable_id" value="{{ $unit->id }}">
</form>

{{-- Standalone per-file delete forms — outside #edit-unit-form for the same reason. --}}
@foreach($unit->files as $file)
<form id="delete-file-form-{{ $file->id }}" method="POST"
