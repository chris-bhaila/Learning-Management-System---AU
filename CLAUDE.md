...
The following three were fixed 2026-07-18, from that date's SECURITY_AUDIT.md (Medium findings). Do not regress any of them.
- **[RESOLVED] `FileController::store()` verifies the authenticated teacher owns the target `fileable_id` before attaching a file.** Resolves the model from `fileable_type`/`fileable_id` and calls `$this->authorize('update', $fileable)` (reuses `CoursePolicy::update()`/`UnitPolicy::update()` polymorphically — same "isAdmin() bypasses, otherwise teacher must own it" rule already used elsewhere, not a second ownership check to keep in sync). A nonexistent `fileable_id` gets a 404 before authorization runs; an existing-but-not-owned one gets redirected with a flash error (this app's `AuthorizationException` handling, not a raw 403 — see `bootstrap/app.php`). Any future write path that accepts a client-supplied `fileable_id`/`course_id`/`unit_id` must get the same treatment — don't assume role-only authorization (`StoreFileRequest::authorize()` style) is sufficient once a specific resource ID is involved. (`app/Http/Controllers/FileController.php:16`)
- **[RESOLVED] Token enrollment is atomic against concurrent requests for the same token.** `EnrollmentController::store()` re-fetches the token with `Token::where('id', $token->id)->lockForUpdate()->first()` inside a `DB::transaction()` immediately before re-checking `isExpired()`, and the eventual `incrementUses()` call happens inside that same transaction — so two simultaneous enrollments against a `max_uses=1` token can no longer both pass the expiry check and both succeed (`uses_count` staying accurate under real concurrency was verified live with `PHP_CLI_SERVER_WORKERS=4` and two genuinely parallel requests, not just sequential ones). The class-token and course-token logic now live in private `joinClass()`/`joinCourse()` helpers called from inside the transaction closure — any future token-consuming code path must go through the same lock-then-check-then-increment sequence, never a bare `isExpired()` read followed later by `incrementUses()`. (`app/Http/Controllers/Student/EnrollmentController.php`)
- **[RESOLVED] Students can no longer see CourseGroup names anywhere.** `student/courses/show.blade.php` no longer computes or renders `$groupName` (both the header badge and the info-panel "Group" row were removed). `Student\CourseController::show()`/`showUnit()` now call `CourseRepositoryInterface::findWithRelationsForStudent()` instead of `findWithRelations()` — a variant that never eager-loads the `'group'` relation at all, so CourseGroup data doesn't reach a student-facing request even before rendering. `findWithRelations()` itself is unchanged and still eager-loads `'group'` for the Teacher/Admin course show pages, which legitimately display and edit it — **any new student-facing course lookup must use `findWithRelationsForStudent()`, never the generic `findWithRelations()`**, or this leak reopens. (`app/Repositories/Eloquent/EloquentCourseRepository.php`, `app/Http/Controllers/Student/CourseController.php`)
